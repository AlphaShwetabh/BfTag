var questions = [
    "When/where did we meet?",
    "Where was our first date?",
    "Where/when was our first kiss?",
    "Who said 'I love you' first?",
    "When is my birthday?",
    "Where was I born?",
    "What color are my eyes?",
    "Which food do I like/hate?",
    "I’m sitting in front of the TV. What’s on the screen?",
    "What’s my favorite band/musician?",
    "When/where did you meet my parents?",
    "What’s my shoe size?",
    "What’s my weirdest habit?",
    "What am I good at?",
    "What am I okay at?",
    "What am I really bad at?",
    "Which sport do I like the most?",
    "What’s my favorite song?",
    "What film always makes me cry?",
    "What drink do I always order?",
    "If I was collecting something, what would it be?",
    "What is something you wish I didn’t do?",
    "Where would I love to travel to?",
    "I’m ordering a pizza. What toppings does it have?",
    "I’m making a sandwich. What’s on it?",
    "If I could, what candy could I eat all day long?",
    "If I could live anywhere in the world, where would I live?",
    "What weird talent do I have?",
    "Where does my family come from?",
    "What am I deathly afraid of?",
    "What’s my nickname?",
    "What is my favorite author?",
    "What’s my favorite book?",
    "What’s my favorite color?",
    "What kind of films do I like?",
    "What is the first thing that I do in the morning?",
    "Complete this sentence: 'My girlfriend/boyfriend is ______'",
    "What’s my middle name?",
    "What’s my best friend’s name?",
    "Where was our first trip together?",
    "How do I drink my coffee/tea?",
    "Who is the dominant one in our relationship?",
    "What’s my favorite video game?",
    "Which of my hobbies do you find the least interesting?",
    "Who usually wins our arguments?",
    "What do we usually argue about?",
    "What’s my favorite clothing item?",
    "How long do I need in the morning to get ready?",
    "Am I a morning or an evening person?",
    "If you could change anything about me, what would it be?",
    "What’s my favorite type of junk food?",
    "I am at the zoo. Where will I spend all of my day?",
    "What’s my favorite city?",
    "Am I a mountain or a beach girl/boy?",
    "Have I ever practiced an instrument? Which one?",
    "What was my first job?",
    "What was/is my least favorite subject in school?",
    "When do I become the biggest fangirl/fanboy?",
    "Where am I on a Friday night?",
    "How do I spend my vacations?",
    "What is my weirdest interest?",
    "What’s my favorite song right now?",
    "What is one thing I love doing but most people don’t?",
    "I’m going to Starbucks. What will I order?",
    "What did you learn from me?",
    "What’s our song?",
    "What TV show do I like that you hate?",
    "Who is the organized one in the relationship?",
    "Who is more jealous?",
    "I won $1,000. What will I spend it on?",
    "Who’s my favorite YouTuber?",
    "What celebrity would I love to meet the most?",
    "If I was an animal, which one would I be?"
];

function resetQuestions() {
    // Reset questions array if all questions have been asked
    questions = [
        "When/where did we meet?",
        "Where was our first date?",
        "Where/when was our first kiss?",
        "Who said 'I love you' first?",
        "When is my birthday?",
        "Where was I born?",
        "What color are my eyes?",
        "Which food do I like/hate?",
        "I’m sitting in front of the TV. What’s on the screen?",
        "What’s my favorite band/musician?",
        "When/where did you meet my parents?",
        "What’s my shoe size?",
        "What’s my weirdest habit?",
        "What am I good at?",
        "What am I okay at?",
        "What am I really bad at?",
        "Which sport do I like the most?",
        "What’s my favorite song?",
        "What film always makes me cry?",
        "What drink do I always order?",
        "If I was collecting something, what would it be?",
        "What is something you wish I didn’t do?",
        "Where would I love to travel to?",
        "I’m ordering a pizza. What toppings does it have?",
        "I’m making a sandwich. What’s on it?",
        "If I could, what candy could I eat all day long?",
        "If I could live anywhere in the world, where would I live?",
        "What weird talent do I have?",
        "Where does my family come from?",
        "What am I deathly afraid of?",
        "What’s my nickname?",
        "What is my favorite author?",
        "What’s my favorite book?",
        "What’s my favorite color?",
        "What kind of films do I like?",
        "What is the first thing that I do in the morning?",
        "Complete this sentence: 'My girlfriend/boyfriend is ______'",
        "What’s my middle name?",
        "What’s my best friend’s name?",
        "Where was our first trip together?",
        "How do I drink my coffee/tea?",
        "Who is the dominant one in our relationship?",
        "What’s my favorite video game?",
        "Which of my hobbies do you find the least interesting?",
        "Who usually wins our arguments?",
        "What do we usually argue about?",
        "What’s my favorite clothing item?",
        "How long do I need in the morning to get ready?",
        "Am I a morning or an evening person?",
        "If you could change anything about me, what would it be?",
        "What’s my favorite type of junk food?",
        "I am at the zoo. Where will I spend all of my day?",
        "What’s my favorite city?",
        "Am I a mountain or a beach girl/boy?",
        "Have I ever practiced an instrument? Which one?",
        "What was my first job?",
        "What was/is my least favorite subject in school?",
        "When do I become the biggest fangirl/fanboy?",
        "Where am I on a Friday night?",
        "How do I spend my vacations?",
        "What is my weirdest interest?",
        "What’s my favorite song right now?",
        "What is one thing I love doing but most people don’t?",
        "I’m going to Starbucks. What will I order?",
        "What did you learn from me?",
        "What’s our song?",
        "What TV show do I like that you hate?",
        "Who is the organized one in the relationship?",
        "Who is more jealous?",
        "I won $1,000. What will I spend it on?",
        "Who’s my favorite YouTuber?",
        "What celebrity would I love to meet the most?",
        "If I was an animal, which one would I be?"
    ];
}

function showRandomQuestion() {
    if (questions.length === 0) {
        resetQuestions();
    }

    var randomIndex = Math.floor(Math.random() * questions.length);
    var randomQuestion = questions.splice(randomIndex, 1)[0];

    var questionContainer = document.getElementById('questionContainer');
    questionContainer.textContent = randomQuestion;
}

function clearQuestion() {
    var questionContainer = document.getElementById('questionContainer');
    questionContainer.textContent = '';
}
